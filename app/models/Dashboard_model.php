...
class Dashboard_model{
    ...
    public function getRequestByOwnerID($id){
        $this->db->query("SELECT property.user_id as 'owner_id', property.propertyname, property.available, property.price, property.image, property.payment_type, propertyreq.*,user.name 
        FROM property JOIN propertyreq ON propertyreq.property_id = property.id JOIN user ON user.id = propertyreq.user_id 
        WHERE property.user_id = :id AND propertyreq.status = 0");
        $this->db->bind(':id', $id);
    ...
    public function getCommentByUserID($id){
        $this->db->query("SELECT comments.*, property.propertyname,property.slug, user.username FROM comments 
        JOIN property on property.id = comments.property_id JOIN user ON user.id = comments.user_id 
        WHERE property.user_id =:id AND comments.created_at >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)");
        $this->db->bind(':id', $id);
        return $this->db->resultSet();

    }
    public function getPaymentStatusByUserID($id){
        $this->db->query("SELECT transactions.*, property.propertyname, property.user_id as owner_id, user.name FROM transactions 
        JOIN property ON property.id = transactions.property_id JOIN user ON user.id = transactions.user_id 
        WHERE property.user_id = :id AND transactions.created_at >= DATE_SUB(CURDATE(), INTERVAL 60 DAY) ORDER BY transactions.id DESC");
        $this->db->bind(':id', $id);
        return $this->db->resultSet();
    }
    ...
    public function getResidentCount($id){
        $this->db->query("SELECT COUNT(property_id) as total_Resident FROM resident JOIN property ON property.id = resident.property_id 
        JOIN user ON user.id = property.user_id WHERE property.user_id =:id");
        $this->db->bind(':id', $id);
    ...
    public function insertPost($category, $user_id, $propertyname, $type, $slug, $price, $location, $region, $available, $facility, $image, $km, $payment_type, $image2, $image3) {
        ...
        $this->db->query("INSERT INTO property (category_id, user_id, propertyname, type, slug, price, location, region_id, available, facility, image, km, payment_type) 
        VALUES (:category_id, :user_id, :propertyname, :type, :slug, :price, :location, :region_id, :available, :facility, :image, :km, :payment_type)");
    ...
    public function editPost($category,$propertyname,$propertyid,$type,$price,$location,$region,$available,$facility,$image,$km,$payment_type,$image2,$image3) {
       ...
       function handleFileUpload2($file, $target_dir, $allowed_types, $max_size) {
        ...
        $this->db->query("UPDATE property SET category_id = :category_id, propertyname = :propertyname, type = :type, price = :price, location = :location, 
        region_id = :region_id, available = :available, facility = :facility, image = :image, km = :km, payment_type = :payment_type WHERE id = :propertyid");
        $this->db->bind(':category_id', $category);
        $this->db->bind(':propertyname', $propertyname);
        $this->db->bind(':type', $type);
        $this->db->bind(':price', $price);
    ...
    public function getPaymentInfo($url){
        $this->db->query("SELECT transactions.*, payment.bankname, payment.transactionnumber, payment.sendername, payment.image, payment.date, payment.created_at,property.propertyname, user.name 
        FROM transactions JOIN payment ON payment.transaction_id = transactions.id JOIN property ON property.id = transactions.property_id JOIN user ON user.id = transactions.user_id 
        WHERE transactions.transactionurl =:url");
    ...
    public function getResidentDetailByUrl($slug){
        $this->db->query("SELECT resident.*, user.username, user.name,property.price,property.payment_type,property.propertyname 
        FROM resident JOIN user ON user.id = resident.user_id JOIN property ON property.id = resident.property_id WHERE resident.url =:url");
